...
There are no serial-port settings to configure. Web Commander always opens the port at the LP-100A’s own values — 115200 baud, 8 data bits, no parity, one stop bit — and applies them itself, regardless of what the port is set to in Windows. This is true whether the meter is found automatically or connected manually.
